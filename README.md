...
- **AI考察 (Claude × ChatGPT 討論)**: Claude の考察 → ChatGPT の検証 → Claude の結論、
  の討論を経て、**結論1文と打ち手だけ**をコンパクトに表示（討論の経過は折りたたみ）。
  追加の質問での壁打ちも可能（`ANTHROPIC_API_KEY` 必須、`OPENAI_API_KEY` 未設定時は Claude 単独）
...
- バーコードスキャン照会は商品一覧ページ（商品 / SKU）に埋め込み。開くとカメラ /
  USB・Bluetooth リーダー・手入力で JAN / SKU を照会し、商品情報と店舗別在庫を表示
- 商品登録で選ぶ**ブランド / カテゴリは商品一覧の「ブランド / カテゴリの管理」から
  ユーザーが任意に追加・削除**できます（使用中の項目は削除不可）
- **商品の基本情報に表示する項目そのものもカスタマイズ可能**（設定 → 商品の基本情報 項目）。
  ブランド / カテゴリ / シーズン / 素材 / 原産国 / 取扱いの表示オンオフと**名称変更**に加え、
  「フィット」「柄」など**任意のカスタム項目を追加**できます。カスタム項目と
  素材・原産国・取扱いには**選択肢**を登録でき（登録するとドロップダウン、空なら自由入力）、
  登録フォームと商品詳細に反映されます（カラー・サイズは SKU の構成要素のため常に表示）
