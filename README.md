...
- **AI考察 (壁打ち)**: 表示中の期間の集計データを Claude が分析し、良い点・課題・打ち手を提案。
  追加の質問を投げて壁打ちを続けられます（`ANTHROPIC_API_KEY` の設定が必要）

### レジ（店頭会計）
- レジページ（`/register`）でバーコードスキャン（カメラ / USB・Bluetooth リーダー / 手入力）
  → カート → 会計までを完結
- 会員番号の照会・ポイント利用・伝票値引き・支払方法（現金 / クレジット / 電子マネー / QR）、
  現金は預り金からお釣りを自動計算
- 会計は POS 連携 API と同じ取り込みロジックを通るため、在庫減算・顧客実績・ランク・
  ポイント付与・LINE 購入通知まで即時反映
- 会計後はレシート印刷ページ（58mm 想定・伝票番号バーコード付き）へ。過去の伝票からも
  「レシート印刷」で再発行可能

### バーコード
- SKU ごとの JAN コード（EAN-13）バーコードを SVG で生成し、値札ラベルとして印刷
  （商品詳細 → 「値札を印刷」。JAN が無い SKU は Code128 で SKU を符号化）
- JAN 未設定の SKU には商品詳細の「JANコードを採番」ボタンで、有効なチェックデジット付きの
  EAN-13 を重複なく一括発行
- スキャンページ（`/scan`）でカメラ読み取り、または USB / Bluetooth リーダー・手入力で
  JAN / SKU を照会し、商品情報と店舗別在庫を表示
