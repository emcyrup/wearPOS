...
- レジページ（`/register`）は**ログインなしで使えます**（店頭のレジ端末でそのまま開ける）。
  ナビの「レジ」は**サイドメニューなしの専用タブ**で開き、すでにレジのタブが開いていれば
  リロードせずそのタブをアクティブにします（会計中のカートが守られます）。
...
- **商品の基本情報に表示する項目そのものもカスタマイズ可能**（設定 → 商品の基本情報 項目）。
  ブランド / カテゴリ / シーズン / 素材 / 原産国 / 取扱いの表示オンオフに加え、
  「フィット」「柄」など**任意のカスタム項目を追加**でき、登録フォームと商品詳細に反映されます
