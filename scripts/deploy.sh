...
# 手順1で自分自身が書き換わっても途中で壊れないよう、全体を関数にして
# 先に読み切らせる (bash は関数を閉じ括弧まで読んでから実行する)
main() {
  cd "$(dirname "$0")/.."
  APP_DIR="$(pwd)"
  ...
  # npx はパッケージが無いと「入れますか?」と聞いて**入力待ちで止まる**ため、
  # PM2 は必ず --no-install で呼ぶ (pm2 は devDependencies に入っている)
  ...
  # 古い環境では pm2 が lockfile に無く、npm ci で消えることがある
  if ! pm2 -v >/dev/null 2>&1; then
    log "pm2 が見つからないため入れ直します"
  ...
  log "5/5 アプリを入れ替え"
  if pm2 describe wearpos >/dev/null 2>&1; then
  ...
  if curl -fsS --max-time 10 "http://127.0.0.1:8026/api/health" >/dev/null; then
    ...
    echo
    echo "⚠️  アプリの応答がありません。ログを確認してください:" >&2
    echo "    cd $APP_DIR && npx --no-install pm2 logs wearpos --lines 100" >&2
    exit 1
  fi
}
